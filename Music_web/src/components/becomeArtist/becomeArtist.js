...
export const becomeArtist = async (appContainer) => {
  if (!appContainer) {
    console.error(
      "Error: Contenedor de la aplicación no encontrado para el formulario de artista."
    );
    return;
  }
  ...
  const artistNameGroup = createInputGroup(
    "text",
    "artistName",
    "Tu Nombre Artístico",
  ...
  photoDropArea.addEventListener("dragleave", () => {
    ...
    photoDropText.textContent =
      "Arrastra tu foto de perfil aquí o haz clic para seleccionar";
  });
  ...
  const loadGenres = async () => {
    genresContainer.innerHTML =
      '<p class="loading_genres">Cargando géneros...</p>';
    try {
      ...
      if (genres.length === 0) {
        genresContainer.innerHTML =
          '<p class="no_genres_message">No hay géneros disponibles.</p>';
        return;
      }
  ...
  submitButton.innerHTML =
    '<i class="fa-solid fa-star"></i> Convertirme en Artista';
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      // Paso 1: Registrar artista sin imagen
      const response = await fetch("http://localhost:3000/artistas/convertir", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          biografia: biography,
          urlFotoArtista: "", // puede ser null también si el backend lo admite
      ...
      if (!response.ok || data.error) {
        console.error(
          "Error al registrar artista:",
          data.message || response.statusText
        );
        error({ message: data.message || "Error al registrar artista." });
        return;
      }

      const artistaId = data.data?.artistaId || data.artista_id;

      // Paso 2: Si hay imagen, subirla aparte
      if (artistaId && selectedPhotoFile) {
        const formData = new FormData();
        formData.append("file", selectedPhotoFile);

        const resUpload = await fetch(
          `http://localhost:3000/artistas/upload/imagen-artista/${artistaId}`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (!resUpload.ok || uploadResult.error) {
          console.warn(
            "Artista registrado, pero error al subir imagen:",
            uploadResult.message
          );
          error({
            message:
              "Artista creado, pero hubo un problema al subir la imagen.",
          });
          // No hacemos return, para seguir con el proceso
        } else {
      ...
      // Éxito total
      success({
        message: `¡Felicidades, ${artistName}! Ahora eres un artista.`,
      });
